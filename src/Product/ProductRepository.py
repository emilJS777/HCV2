...
class ProductRepository(Repository, IProductRepo):
    ...
    def get_all(self, page: int, per_page: int, product_type_id: int or None, storage_id: int or None, client_id: int) -> dict:
        products = self.product.query\
            ...
            .filter(self.product.storage_id == storage_id if storage_id else self.product.storage_id.isnot(None))\
            .join(self.product.storage)\
            .filter(Storage.firm_id.in_(permission.firm_id for permission in g.user.firm_permissions))\
