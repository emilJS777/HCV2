...
class FirmRepository(Repository, IFirmRepo):
    ...
    def get_by_id(self, firm_id: int, client_id: int) -> dict:
        firm = self.firm.query.filter_by(id=firm_id, client_id=client_id).first()
        firm.sphere = firm.sphere
        return firm

    def get_all(self, page: int, per_page: int, client_id: int) -> dict:
        firms = self.firm.query.filter_by(client_id=client_id).paginate(page=page, per_page=per_page)
