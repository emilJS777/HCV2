...
class ResourceController(Controller):
    resource_service: ResourceService = ResourceService(ResourceRepository())

    # POST
    @expects_json(resource_schema)
    @AuthMiddleware.check_authorize
    @PermissionMiddleware.check_permission('resource_edit')
    def post(self) -> dict:
    ...
    # GET
    @AuthMiddleware.check_authorize
    @PermissionMiddleware.check_permission('resource_get')
    def get(self) -> dict:
        if self.id:
            res: dict = self.resource_service.get_by_id(self.id)
        else:
            res: dict = self.resource_service.get_all(page=self.page, per_page=self.per_page)
        return res
